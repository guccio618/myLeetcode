...
public class Q272_Closest_Binary_Search_Tree_Value_II {
    ...
    public List<Integer> closestKValues(TreeNode root, double target, int k) {
        ...
        while(!stack.isEmpty() || root != null){
            ...
            root = stack.pop();
            heap.offer(new Node(root.value, Math.abs((double) root.value - target)));
            root = root.right;
        }
	...
	public List<Integer> closestKValues2(TreeNode root, double target, int k) {
        ...
        if(!pred.isEmpty() && !succ.isEmpty() && pred.peek().value == succ.peek().value){   // 注意这一步必须有 ！！！
        ...
        while(k-- > 0){
            if(succ.isEmpty()){
                ...
                double succ_diff = Math.abs((double)succ.peek().value - target);
                double pred_diff = Math.abs((double)pred.peek().value - target);
